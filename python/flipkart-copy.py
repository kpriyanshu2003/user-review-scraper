        ...
        try:
            if end_page is None:
                ...
                page_html = BeautifulSoup(
                    no_page.get_attribute("outerHTML"), "html.parser"
                )
                page_content = (
        ...
        try:
            for j in range(start_page, end_page + 1):
                ...
                content_div = driver.find_element(
                    By.CSS_SELECTOR, ".DOjaWF.gdgoEp.col-9-12"
                )
                ...
                for i in reviews:
                    ...
                    extract_review_rating = lambda i: (
                        i.find("div", class_="XQDdHH Ga3i8K")
                        .get_text()
                        .strip()
                        .split()[0]
            ...
            while True:
                if os.path.isfile(f"./fpp/{product_id}.csv"):
                    product_id = product_id + "-" + str(int(uniform(1, 100)))
                else:
                    break
            csv_filename = f"./fpp/{product_id}.csv"
